...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            $.ajax({
                type: "post",
                url: "${pageContext.request.contextPath}/query/queryMenu",
                dataType: "json",
                success: function (result) {  //遍历集合
                    $.each(result, function (index, first) {
                        var c = "";
                        $.each(first.menus, function (index1, second) {
                            c += "<p style='text-align: center'><a href='#' class='easyui-linkbutton' onclick=\"addTabs('" + second.iconCls + "','" + second.content + "','" + second.href + "')\" data-options=\"iconCls:'icon-tip'\">" + second.content + "</a></p>";
                        })
                        $('#aa').accordion('add', {//用遍历把手风琴的标题和内容展示出来
                            title: first.title,
                            content: c,
                            selected: false,
        ...
        function addTabs(iconCls, title, href) {
            ...
            var flag = $('#tt').tabs("exists", title); //判断选项卡是否存在
            if (flag) {
                $('#tt').tabs("select", title); //存在,选择title的这个标签
            } else {
                $('#tt').tabs('add', { //不存在就创建
                    title: title,
                    selected: true,
